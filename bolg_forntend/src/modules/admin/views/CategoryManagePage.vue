...
import { adminGetCategories, adminCreateCategory, adminUpdateCategory, adminDeleteCategory } from '../../../shared/api/modules/category'
import ConfirmModal from '../../../components/ConfirmModal.vue'
import { useMessage } from '../../../composables/useMessage'

const message = useMessage()
const categories = ref([])
const form = ref({ name: '', slug: '', description: '', parent_id: 0 })
...
function openCreate() {
  form.value = { name: '', slug: '', description: '', parent_id: 0 }
  editId.value = null
  showForm.value = true
}

function openEdit(c) {
  form.value = { name: c.name, slug: c.slug, description: c.description || '', parent_id: c.parent_id || 0 }
  editId.value = c.id
  showForm.value = true
}
...
async function handleSubmit() {
  try {
    if (editId.value) {
      await adminUpdateCategory(editId.value, form.value)
    } else {
    ...
    load()
  } catch (e) { message.error('操作失败: ' + (e.response?.data?.msg || e.message)) }
}
...
async function doDelete() {
  ...
  try {
    await adminDeleteCategory(deleteId.value)
...
<template>
  <div>
    <div class="page-header"><h3>分类管理</h3><button class="primary-btn" @click="openCreate">+ 新建分类</button></div>
    <table class="data-table">
      <thead><tr><th>名称</th><th>Slug</th><th>描述</th><th>操作</th></tr></thead>
      <tbody>
        <tr v-for="c in categories" :key="c.id">
          <td>{{ c.name }}</td><td>{{ c.slug }}</td><td>{{ c.description || '-' }}</td>
          <td class="actions"><button @click="openEdit(c)">编辑</button><button class="danger" @click="handleDelete(c.id)">删除</button></td>
        </tr>
      </tbody>
    </table>

    <div v-if="showForm" class="modal-overlay" @click.self="showForm=false">
      <div class="modal"><h4>{{ editId ? '编辑分类' : '新建分类' }}</h4>
        <form @submit.prevent="handleSubmit">
          <label>名称 <input v-model="form.name" required /></label>
          <label>Slug <input v-model="form.slug" required /></label>
          <label>描述 <input v-model="form.description" /></label>
          <div class="form-actions"><button type="submit" class="primary-btn">保存</button><button type="button" @click="showForm=false">取消</button></div>
        </form>
      </div>
    </div>
...
.modal input { width: 100%; padding: 8px 12px; border: 1px solid #ddd; border-radius: 6px; margin-top: 4px; box-sizing: border-box; }
